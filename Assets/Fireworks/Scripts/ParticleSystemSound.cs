	...
	public void LateUpdate() {
		...
		while (i < length){
	 		if(_explosionSound.Length > 0 && particles[i].remainingLifetime < Time.deltaTime){
	 			SoundController.instance.Play(_explosionSound[UnityEngine.Random.Range(0, _explosionSound.Length)], UnityEngine.Random.Range(_explosionVolumeMin, _explosionVolumeMax), UnityEngine.Random.Range(_explosionPitchMin,_explosionPitchMax), particles[i].position);
