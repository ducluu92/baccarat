...
namespace Assets.Scripts.Pages
{
    public static class NoticePage
    {
        ...
        private static List<NoticeData> _origin;

        public static Paging<NoticeData> Data { get; private set; }

        // 1번 인덱스로 초기화
        public static void SetByData(List<NoticeData> origin) 
        {
            _origin = origin;
            var query = origin.OrderByDescending(c => c.Index).AsQueryable();
            Data = PageingBuilder<NoticeData>.Build(query, pageSize, pagingSize);
        }
